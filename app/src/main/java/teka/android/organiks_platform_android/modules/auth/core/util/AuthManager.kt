package teka.android.organiks_platform_android.modules.auth.core.util

import android.annotation.SuppressLint
import android.content.Context
import teka.android.organiks_platform_android.modules.auth.core.data.remote.CustomAuthService
import teka.android.organiks_platform_android.modules.auth.core.repository.DataStoreRepository
...
sealed class LoginResultModel {
    data class Success(val isLoggedIn: Boolean, val roles:List<UserRoleDto>?) : LoginResultModel()
            ...
            if (e is HttpException) {
                ...
                if (errorResponse != null) {
                    ...
                    return RegistrationResult.Failure(
                        Exception(
                            errorResponse.toString() ?: "Unknown error"
                        )
                    )
                }else{
        ...
        return try {
            ...
            if (response.isSuccessful) {
                ...
                return LoginResultModel.Failure(
                    Exception(response.status ?: "Login Failed"),
                    errorMessage
                )
            }
        } catch (e: Exception) {
