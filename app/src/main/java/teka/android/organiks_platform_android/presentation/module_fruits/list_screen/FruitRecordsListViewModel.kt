...
const val GateLogTab_VM_TAG = "GateLogTab_VM_TAG"

@HiltViewModel
class GateLogsTabViewModel @Inject constructor(
    private val appContext: Context,
    private val dbRepository: DbRepository,
) : ViewModel() {
    // UI state holder
    private val _fruitRecordsUIState = MutableStateFlow(FruitRecordsUIState())
    val fruitRecordsUIState: StateFlow<FruitRecordsUIState> = _fruitRecordsUIState


    init {
        Timber.tag(GateLogTab_VM_TAG).i("init function now")
        observeVehicleList()
    }
    ...
    fun onDateRangeSelected(selectedRange: Pair<Long?, Long?>) {
        Timber.tag(GateLogTab_VM_TAG).i("selectedDateRange:: $selectedRange")
    ...
    fun <T> updateModelField(property: KMutableProperty1<FruitRecordsUIState, T>, value: T) {
        _fruitRecordsUIState.update { currentState ->
